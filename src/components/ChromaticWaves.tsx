// Originkit "Chromatic Waves" (https://originkit.dev), adapted from its
// Framer form only by swapping the HTTPS ESM ogl import (Framer resolves
// those at build) for the local npm 'ogl' package — the effect itself
// (Perlin rainbow field resolved into a dot grid) is delivered verbatim.
...
export default function ChromaticWaves(props: ChromaticWavesProps) {
    const {
        frequency = 1,
        speed = 4,
        bgColor = 'transparent',
    ...
    useEffect(() => {
        ...
        if (!isPlayingRef.current) renderOnce();
    }, [effectivePlay, frequency, speed, effPaletteCount, bgColor, paletteKey, cellSize, gamma, paletteBias]);
