// Originkit "Mesh Text Hover" (https://originkit.dev), adapted only by
// typing the loose `props: any` entry point — the WebGL mesh-warp + canvas
// text-render engine is delivered verbatim, no external deps (raw WebGL2,
// no ogl). Renders `text` onto an offscreen canvas as a texture, then warps
// a subdivided grid mesh under the cursor with spring physics, with an
// optional chromatic (RGB-split) fringe on the displaced edges.
"use client"
...
export interface MeshTextHoverProps {
    text: string
    color?: string
    fontFamily?: string
    fontWeight?: number
    ...
    fontSize?: number
    /** Rueda de colores para el fleco cromático (RGB-split) en los bordes desplazados. */
    colorSplit?: boolean
    customColors?: string[]
    /** Fuerza del arrastre del mouse sobre la malla, escala UI 0..50 (default 18). */
    force?: number
    className?: string
    style?: CSSProperties
}
...
export default function MeshTextHover({
    ...
    useEffect(() => {
        ...
        // The mesh's own NDC space is a plain square (-1..1 on both axes)
        // regardless of the container's actual aspect ratio. A "circular"
        // proximity radius in that square space becomes a razor-thin band
        // on screen once the box is much wider than tall (a full-width
        // wordmark strip), which is why vertical drag felt dead. Track the
        // real aspect and un-stretch the y-axis before measuring distance
        // so the interactive zone reads as a circle on screen, not in NDC.
        ...
        const tick = () => {
            ...
            for (let i = 0; i < vertCount; i++) {
                ...
                const cy = (cursor.y - (py + dy)) / aspectRef.current
            ...
            if (cols.length === 1) {
                ...
                const cycleMs = 400
                const idx2 = Math.floor(performance.now() / cycleMs) % cols.length
                cA = cols[idx2]
                cB = cols[(idx2 + 1) % cols.length]
            }
    ...
    return (
        <div
            ref={wrapperRef}
            className={className}
            style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", userSelect: "none", ...style }}
        >
