      ...
      {project && (
        <div className="fixed inset-0 z-50 flex items-end md:items-stretch justify-center md:justify-end overflow-hidden">
          ...
          <motion.div
            ...
            className="absolute inset-0 bg-black/60 md:block hidden"
            onClick={onClose}
          />
          {/* Mobile tap-outside */}
          {isMobile && (
              ...
              <div className={isMobile ? 'p-5 space-y-8 pb-12' : 'p-12 space-y-12'}>
                ...
                <div className={`w-full overflow-hidden relative bg-zinc-100 ${isMobile ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
                  <motion.img
                    ...
                    decoding="async"
                    animate={{ y: ['0%', '-20%'] }}
                    transition={{ duration: 18, ease: 'easeInOut', repeat: Infinity, repeatType: 'reverse', delay: 0.6 }}
                    style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '130%', objectFit: 'cover', objectPosition: 'top center', willChange: 'transform' }}
                  />
                </div>

                {/* Title */}
                <div className="space-y-4">
                  <h2 className={`font-serif font-black tracking-tight text-[#1a1a1a] ${isMobile ? 'text-3xl' : 'text-4xl md:text-5xl'}`}>
                    {project.title}
                  </h2>
                  <p className={`font-serif font-light text-zinc-600 leading-relaxed ${isMobile ? 'text-base' : 'text-lg'}`}>
                    {project.description}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <h4 className="font-serif text-base font-semibold text-[#1a1a1a]">Filosofía de Diseño</h4>
                  <p className="font-sans text-[#1a1a1a]/80 leading-[1.8] text-sm">{project.detailedDescription}</p>
                </div>

                {/* Services */}
                <div className="space-y-3">
                  <h4 className="font-serif text-base font-semibold text-[#1a1a1a]">Servicios & Entregables</h4>
